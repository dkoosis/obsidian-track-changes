...
function parseMetaPrefix(prefix: string): MetaPrefix {
  ...
  if (prefix) {
    for (const m of prefix.matchAll(META_PAIR_RE)) {
      const key = m[1]!.toLowerCase(); // META_PAIR_RE always captures groups 1 and 2
...
function collectCandidates(text: string): CriticNode[] {
  ...
  for (const m of text.matchAll(SUBSTITUTION_RE)) {
    // Groups fixed by SUBSTITUTION_RE: 1 = prefix, 2 = oldText, 3 = newText.
    const prefix = m[1]!, oldText = m[2]!, newText = m[3]!;
    const meta = parseMetaPrefix(prefix);
    const from = m.index;
    const innerFrom = from + 1 + prefix.length + 2; // {<prefix>~~
    out.push({
      kind: "substitution",
      from,
      to: from + m[0].length,
      ...
      metaRaw: prefix,
      innerFrom,
      innerTo: innerFrom + oldText.length,
      oldText,
      newText,
    });
  }
  for (const m of text.matchAll(ADDITION_RE)) {
    const prefix = m[1]!, text2 = m[2]!; // groups: 1 = prefix, 2 = text
    const meta = parseMetaPrefix(prefix);
    const from = m.index;
    const innerFrom = from + 1 + prefix.length + 2; // {<prefix>++
    out.push({
      ...
      metaRaw: prefix,
      innerFrom,
      innerTo: innerFrom + text2.length,
      text: text2,
    });
  }
  ...
  for (const m of text.matchAll(COMMENT_RE)) {
    const raw = m[0];
    const prefix = m[1]!, body = m[2]!; // groups: 1 = prefix, 2 = body
    const meta = parseMetaPrefix(prefix);
    const from = m.index;
    const bodyStart = from + 1 + prefix.length + 2; // {<prefix>>>
